...
void BoundingBox::swim(glm::vec3 direction, float power) {
	float currentTime = glfwGetTime();
	if ((currentTime - lastMovement) > 0.1f) {
		lastMovement = currentTime;
		lastMovementDirection = glm::normalize(direction);
...
//calculates the displacement of the bounding box based on the current acceleration, velocity and whether or not gravity is being applied to it.
glm::mat4 BoundingBox::getMovementTransform() {
	...
	if(velocityStrength != 0.0f){
		glm::vec3 unitVelocity = velocity / velocityStrength;
		 velocity = velocity - ((float)glm::pow(velocityStrength, 2) * resistancePower * unitVelocity);
	}	
	
	//std::cout << glm::to_string(velocity);

	float currentTime = glfwGetTime();
	// this is used for movement made due to user input
	if ((currentTime - lastMovement) > 0.01f) {		
		acceleration -= lastMovementDirection * lastMovementPower;	
		lastMovementDirection = glm::vec3(0.0f, 0.0f, 0.0f);
		lastMovementPower = 0.0f;
	}
	//should check for collisions in other directions without checking whether gravity is on or not
	if (gravity) {
		for (int i = 0; i < boundingBoxes.size(); i++) {
			if (this != boundingBoxes[i] && this->doesCollideOld(*boundingBoxes[i], glm::vec3(min.x, min.y - 0.1f, min.z), glm::vec3(max.x, max.y - 0.1f, max.z))) {
				if (gravityEnabled) {
