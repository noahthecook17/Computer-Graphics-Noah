...
void processInput(GLFWwindow* window, bool cameraDisabled)
{
...
public:
    static int run() {
        ...
        diverModel.initialise(90, 0, -90, glm::vec3(0.0f, 0.0f, -1.0f));

        PhysicsModel* diver = diverModel.getTestBox(true, true);
        diver->translateModel(glm::vec3(-5.0f, 30.0f, 0.0f));

        //PhysicsModel* diver2 = diverModel.getTestBox(false, true);
        

        Model floorModel("floor.obj");

        floorModel.initialise(0, 0, 0, glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, 0.0f, 0.0f), 30.0f);

        PhysicsModel* floor = floorModel.getTestBox(false, true);

        Model cubeModel("badcube.obj");

        cubeModel.initialise(0, 0, 0, glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, 30.0f, 0.0f), 1.0f);
        ...
        ourModel.initialise(0, 45.0f, 0, glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(-10.0f, 5.0f, -20.0f), 0.2f);
        ...
        PhysicsModel * fishy = new PhysicsModel(ourModel.meshes, false, true);

        //fishy2->rotate(45, glm::vec3(0, 0, 1));        
        
        //fishy->scale(0.25f);
        //fishy->rotate(180, glm::vec3(0, 0, 1));
        //fishy->rotate(90, glm::vec3(0, 1, 0));
        //fishy->rotate(30, glm::vec3(0, 0, 1));
        ...
        //Shader shadowProgram ("shadowMapVert.glsl", "shadowMapFrag.glsl", nullptr);
        ...
        while (!glfwWindowShouldClose(window)) {  
            ...
            fishy1->swimRight(100.0f);

            glEnable(GL_DEPTH_TEST);
            glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

            
            camera.Position = glm::vec3(diver->getOBB()->getCenter().x, diver->getOBB()->getCenter().y + 2.0f, diver->getOBB()->getCenter().z + 10.0f);
            ...
            */
            ...
            glBindTexture(GL_TEXTURE_2D, texture);
            cubeModel.Draw(ourShader, false);
            ...
            //glUniformMatrix4fv(modelUniform, 1, GL_FALSE, glm::value_ptr(diverOBB->getModelTransform()));
            //glUniformMatrix4fv(modelUniform, 1, GL_FALSE, glm::value_ptr(diver->getOBB()->getModelTransform()));
            //ourShader.setMat4("model", glm::value_ptr(diver->getModelTransform()));
            diverModel.Draw(ourShader, false);
           ...
           // glUniformMatrix4fv(modelUniform, 1, GL_FALSE, glm::value_ptr(model3->getTransform()));
            ...
            floorModel.Draw(ourShader, false);
